function initializeBlacklist() {
    ...
    async function render() {
        ...
        listEl.querySelectorAll('.fp-bl-delivery, .fp-bl-response, .fp-bl-notif').forEach(cb => {
            cb.addEventListener('change', async () => {
                const { fpToolsBlacklist: bl = [] } = await browser.storage.local.get('fpToolsBlacklist');
                ...
                await browser.storage.local.set({ fpToolsBlacklist: bl });
            });
        });
    ...
    addBtn.addEventListener('click', async () => {
        ...
        await browser.storage.local.set({ fpToolsBlacklist });
