...
// Функция для сохранения статусов в browser.storage.local
async function saveUserStatuses() {
    await browser.storage.local.set({ fpToolsUserStatuses: userStatuses });
}
...
async function addStatusButtonsToChatMenu() {
    const observer = new MutationObserver(async (mutationsList, obs) => {
        ...
        if (chatMenu && !chatMenu.dataset.fpToolsStatusMenu) {
            ...
            ] } = await browser.storage.local.get('fpToolsCustomLabels');

            const saveLabels = () => browser.storage.local.set({ fpToolsCustomLabels });
...
async function initializeUserNotes() {
    const { fpToolsUserStatuses } = await browser.storage.local.get('fpToolsUserStatuses');
