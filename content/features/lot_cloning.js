...
async function checkForCopiedLotData() {
    ...
    const result = await browser.storage.local.get(COPIED_LOT_STORAGE_KEY);
    ...
    if (!copiedData || (Date.now() - copiedData.timestamp > 10 * 60 * 1000)) {
        await browser.storage.local.remove(COPIED_LOT_STORAGE_KEY);
        return;
    }
    ...
    document.getElementById('paste-lot-data-btn').addEventListener('click', () => {
        ...
        browser.storage.local.remove(COPIED_LOT_STORAGE_KEY);
        pasteBar.remove();
    });
    ...
    if (!document.querySelector('.fp-tools-import-btn')) {
        ...
        importButton.addEventListener('click', async () => {
            ...
            try {
                ...
                const lots = await browser.runtime.sendMessage({ action: 'getUserLotsList', userId: userId });
...
function initializeGlobalSearchLogic() {
    ...
    searchInput.addEventListener('input', () => {
        ...
        debounceTimer = setTimeout(async () => {
            ...
            try {
                const games = await browser.runtime.sendMessage({ action: 'searchGames', query: query });
    ...
    resultsContainer.addEventListener('click', async (e) => {
        ...
        try {
            if (searchState.step === 'game') {
                ...
                const categories = await browser.runtime.sendMessage({ action: 'getCategoryList', url: searchState.gameUrl });
                ...
                const lots = await browser.runtime.sendMessage({ action: 'getLotList', url: searchState.categoryUrl });
