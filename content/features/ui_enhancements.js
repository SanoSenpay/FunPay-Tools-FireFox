        ...
        try {
            ...
            if (resp.ok) {
                ...
                const result = await new Promise(r =>
                    browser.runtime.sendMessage({ target: 'offscreen', action: 'parseUnconfirmedBalance', html }, d => r(d))
                );
                if (result) {
...
function initializeSalesStatistics() {
    ...
    updateBtn.addEventListener('click', async () => {
        if (!browser.runtime?.id) return; // Защита от недействительного контекста
        updateBtn.disabled = true;
        updateBtn.textContent = "Обновление...";
        await browser.runtime.sendMessage({ action: 'resetSalesStorage' });
        await browser.runtime.sendMessage({ action: 'updateSales' });
    });
    
    displaySalesStats();
    if (browser.runtime?.id) {
        browser.runtime.sendMessage({ action: 'updateSales' });
    }
