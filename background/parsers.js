...
function parseLotEditPage(html) {
    try {
        ...
        // Firefox Fix: FormData(form) does NOT capture field values from DOMParser-parsed docs
        // (the DOM is inert — no live form association). We manually walk every field.
        const dataObject = {};

        // <input> — all types except submit/button/image/reset
        form.querySelectorAll('input').forEach(el => {
            if (!el.name) return;
            const type = (el.type || 'text').toLowerCase();
            if (['submit', 'button', 'image', 'reset'].includes(type)) return;
            if (type === 'checkbox' || type === 'radio') {
                // Include only checked boxes/radios
                if (el.checked) dataObject[el.name] = el.value;
            } else if (type === 'file') {
                // Skip file inputs — can't transfer binary from background
            } else {
        ...
        form.querySelectorAll('textarea').forEach(el => {
            if (el.name) dataObject[el.name] = el.value;
        });

        // <select> — honour multiple selects
        form.querySelectorAll('select').forEach(el => {
            if (!el.name) return;
            if (el.multiple) {
                Array.from(el.selectedOptions).forEach(opt => {
                    // Append multiple values under the same key
                    if (dataObject[el.name] === undefined) {
                        dataObject[el.name] = opt.value;
                    } else {
                        // Store as array-like — URLSearchParams will handle repeated keys
                        if (!Array.isArray(dataObject[el.name])) {
                            dataObject[el.name] = [dataObject[el.name]];
                        }
                        dataObject[el.name].push(opt.value);
                    }
                });
            } else {
                dataObject[el.name] = el.value;
        ...
        // Remove location — it's machine-specific and breaks import on a different account
        delete dataObject.location;

        return dataObject;
    } catch (e) {
        console.error("FP Tools: Error in parseLotEditPage", e);
        return null;
    }
}
