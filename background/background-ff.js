...
// =============================================================
// Firefox: центральный диспетчер парсинга (замена offscreen API)
// =============================================================
...
async function runSalesUpdateCycle() {
    ...
    try {
        ...
        } = await browser.storage.local.get(['fpToolsSalesData', 'fpToolsFirstOrderId', 'fpToolsLastOrderId']);
        ...
        await browser.storage.local.set({ fpToolsSalesLastUpdate: Date.now() });
    }
}
...
// Firefox: DOMParser доступен напрямую в background — offscreen не нужен
function parseHtmlViaOffscreen(html, action) {
    return Promise.resolve(parseHtmlInBackground(html, action));
}
...
async function runDiscordCheckCycle() {
    const { fpToolsDiscord, fpToolsProcessedDiscordIds } = await browser.storage.local.get(['fpToolsDiscord', 'fpToolsProcessedDiscordIds']);

    if (!fpToolsDiscord || !fpToolsDiscord.enabled || !fpToolsDiscord.webhookUrl) {
        browser.alarms.clear(DISCORD_LOG_ALARM_NAME);
        return;
    }
    ...
    try {
        ...
        if (newMessagesToSend) {
            ...
            await browser.storage.local.set({ fpToolsProcessedDiscordIds: idsToStore });
        }
...
async function processNextLotImport() {
    ...
    if (!process || process.state === 'postponed' || process.currentIndex >= process.lots.length) {
        if (process && process.currentIndex >= process.lots.length) {
            await browser.storage.local.remove(IMPORT_PROCESS_KEY);
    ...
    if (currentLot.status === 'success' || currentLot.status === 'skipped') {
        ...
        await browser.storage.local.set({ [IMPORT_PROCESS_KEY]: process });
    ...
    try {
        ...
        // Firefox Fix: lot.data may contain array values from multi-select fields.
        // new URLSearchParams(obj) ignores arrays, so build params manually.
        const formData = new URLSearchParams();
        for (const [key, value] of Object.entries(currentLot.data)) {
...
browser.runtime.onMessage.addListener((request, sender, sendResponse) => {
    // Firefox: парсинг выполняется прямо здесь, offscreen не нужен
    if (request.target === 'offscreen') {
        try {
            sendResponse(parseHtmlInBackground(request.html, request.action));
        } catch(e) {
            sendResponse(null);
        }
        return true;
    ...
    if (request.action === 'saveSingleLot') {
        (async () => {
            try {
                ...
                // Firefox Fix: handle possible array values from multi-select fields
                const formData = new URLSearchParams();
    ...
    if (request.action === 'startLotImport') {
        (async () => {
            ...
            await browser.storage.local.set({ [IMPORT_PROCESS_KEY]: importProcess });
            sendResponse({ success: true });
    ...
    if (request.action === 'cancelLotImport') {
        browser.storage.local.remove(IMPORT_PROCESS_KEY).then(() => sendResponse({success: true}));
        return true;
    }
    ...
    if (request.action === 'getGoldenKey') {
        (async () => {
            const cookie = await browser.cookies.get({ url: "https://funpay.com", name: "golden_key" });
    ...
    if (request.action === 'setGoldenKey') {
        (async () => {
            ...
            const sessionCookie = await browser.cookies.get({ url: 'https://funpay.com', name: 'PHPSESSID' });
            if (sessionCookie) {
                await browser.cookies.remove({ url: 'https://funpay.com', name: 'PHPSESSID' });
            }
    ...
    if (request.action === 'deleteCookiesAndReload') {
        (async () => {
            const allCookies = await browser.cookies.getAll({ url: "https://funpay.com" });
            for (const cookie of allCookies) {
                await browser.cookies.remove({ url: "https://funpay.com", name: cookie.name });
            }
            browser.tabs.reload(sender.tab.id);
        })();
        return true;
    ...
    if (request.action === 'supportGetTickets' || request.action === 'supportGetCategories' ||
        ...
        (async () => {
            try {
                const gkCookie = await browser.cookies.get({ url: 'https://funpay.com', name: 'golden_key' });
                const phpCookie = await browser.cookies.get({ url: 'https://funpay.com', name: 'PHPSESSID' });
...
browser.alarms.onAlarm.addListener(async (alarm) => {
    ...
    if (alarm.name === 'fpToolsAutoRestore') {
        ...
        const tabs = await browser.tabs.query({ url: "https://funpay.com/*" });
        tabs.forEach(tab => {
            browser.tabs.sendMessage(tab.id, { action: 'fpToolsCheckRestoreLots' }).catch(() => {});
        });
    }
});

function setupInitialAlarms() {
    browser.storage.local.get(['autoBumpEnabled', 'autoBumpCooldown', 'fpToolsDiscord', 'fpToolsAutoReplies'], (settings) => {
        if (settings.autoBumpEnabled && settings.autoBumpCooldown) {
            browser.alarms.create(BUMP_ALARM_NAME, {
                delayInMinutes: 1,
        ...
        if (settings.fpToolsAutoRestoreEnabled || settings.fpToolsAutoDisableEnabled) {
            browser.alarms.create(AUTO_RESTORE_ALARM, {
                delayInMinutes: 1,
                periodInMinutes: 5
            });
        }
        ...
        if (settings.fpToolsDiscord && settings.fpToolsDiscord.enabled && settings.fpToolsDiscord.webhookUrl) {
            browser.alarms.create(DISCORD_LOG_ALARM_NAME, {
                delayInMinutes: 1,
                periodInMinutes: 1
            });
        ...
        if (arAnyEnabled) {
            browser.alarms.create(AUTO_RESPONDER_ALARM_NAME, {
                delayInMinutes: 1,
...
browser.runtime.onStartup.addListener(setupInitialAlarms);
...
browser.storage.onChanged.addListener((changes, area) => {
    ...
    if (changes.fpToolsDiscord) {
        ...
        browser.alarms.get(DISCORD_LOG_ALARM_NAME, (alarm) => {
            if (isEnabled && !alarm) {
                browser.alarms.create(DISCORD_LOG_ALARM_NAME, { delayInMinutes: 1, periodInMinutes: 1 });
                runDiscordCheckCycle();
    ...
    if (changes.fpToolsAutoReplies) {
        ...
        browser.alarms.get(AUTO_RESPONDER_ALARM_NAME, (alarm) => {
            if (isEnabled && !alarm) {
                browser.alarms.create(AUTO_RESPONDER_ALARM_NAME, { delayInMinutes: 1, periodInMinutes: 0.25 });
                runAutoResponderCycle();
            } else if (!isEnabled && alarm) {
                browser.alarms.clear(AUTO_RESPONDER_ALARM_NAME);
