...
async function getAuth() {
    ...
    for (const tab of tabs) {
        ...
        try {
            const r = await browser.tabs.sendMessage(tab.id, { action: 'getAppData' });
            if (r?.success) {
...
function parseViaOffscreen(html, action) {
    // Firefox: DOMParser is available directly in background pages
...
async function isBlacklisted(username, feature) {
    const { fpToolsBlacklist = [] } = await browser.storage.local.get('fpToolsBlacklist');
...
async function runAutoResponderCycle() {
    const { fpToolsAutoReplies = {} } = await browser.storage.local.get('fpToolsAutoReplies');
    ...
    const { [RUNNER_TAG_KEY]: savedTag } = await browser.storage.local.get(RUNNER_TAG_KEY);
    ...
    try {
        ...
        if (chatObj.tag) await browser.storage.local.set({ [RUNNER_TAG_KEY]: chatObj.tag });
        if (!chatObj.data?.html) return;

        const chats = await parseViaOffscreen(chatObj.data.html, 'parseChatList');
        const { fpToolsAutoReplies: fresh = {} } = await browser.storage.local.get('fpToolsAutoReplies');
...
async function resetAutoResponderState() {
    await browser.storage.local.remove(RUNNER_TAG_KEY);
}